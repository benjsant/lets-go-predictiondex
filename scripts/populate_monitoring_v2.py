...
def get_pokemon_moves(pokemon_id: int, headers: Dict) -> List[str]:
    ...
    try:
        ...
        print(f"⚠️  Error retrieving moves for Pokemon {pokemon_id}: {exc}")
        return []
...
def generate_predictions(n=50):
    ...
    if len(pokemon_list) < 10:
        print(f"⚠️  Only {len(pokemon_list)} Pokemon available")
        return 0
    ...
    for i in range(n):
        ...
        try:
            ...
            errors += 1
            if errors <= 3:
                print(f"⚠️  Exception: {str(exc)[:100]}")
    ...
    if skipped > 0:
        print(f"⏭️  {skipped} predictions skipped (not enough moves)")
    if errors > 0:
...
def main():
    ...
    if success < args.count * 0.3:
        print("\n⚠️  Few predictions succeeded. Pokemon data may be incomplete.")
