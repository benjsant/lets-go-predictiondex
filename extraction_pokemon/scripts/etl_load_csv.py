...
import extraction_pokemon.models  # noqa: F401
...
from extraction_pokemon.db.guards.type import upsert_type
from extraction_pokemon.db.guards.move import upsert_move
from extraction_pokemon.db.guards.pokemon import upsert_pokemon
from extraction_pokemon.db.guards.pokemon_type import upsert_pokemon_type

DATA_PATH = "extraction_pokemon/data/csv"
...
def load_moves(session):
    ...
    print("➡ Loading moves...")
    type_map = {normalize_key(t.name): t.id for t in session.query(extraction_pokemon.models.Type).all()}
    category_map = {normalize_key(c.name): c.id for c in session.query(extraction_pokemon.models.MoveCategory).all()}
...
def load_pokemon_types(session):
    ...
    type_map = {
        ...
        for t in session.query(extraction_pokemon.models.Type).all()
    }
    ...
    with open(f"{DATA_PATH}/liste_pokemon.csv", encoding="utf-8") as file:
        ...
        for row in reader:
            ...
            pokemon = (
                session.query(extraction_pokemon.models.Pokemon)
                .join(Form)
                .filter(extraction_pokemon.models.Pokemon.species_id == species_id)
